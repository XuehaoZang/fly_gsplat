...
class SplatfactoModelConfig(ModelConfig):
    ...
    position_reg_lambda: float = 0.01
    """Weight for position regularization loss pulling gaussians toward initialization."""
...
class SplatfactoModel(Model):
    ...
    def populate_modules(self):
        if self.seed_points is not None and not self.config.random_init:
            ...
            self.means_init = self.seed_points[0].clone().detach()
        else:
            means = torch.nn.Parameter((torch.rand((self.config.num_random, 3)) - 0.5) * self.config.random_scale)
            self.means_init = means.data.clone().detach()
    ...
    def get_loss_dict(self, outputs, batch, metrics_dict=None) -> Dict[str, torch.Tensor]:
        ...
        # Position regularization: pull means toward initialization
        pos_reg = torch.tensor(0.0).to(self.device)
        if hasattr(self, 'means_init'):
            pos_reg = self.config.position_reg_lambda * (self.means - self.means_init.to(self.device)).pow(2).mean()
        loss_dict = {
...
@dataclass
class SplatfactoCustomModelConfig(SplatfactoModelConfig):
    """Custom Splatfacto with position regularization."""
    _target: Type = field(default_factory=lambda: SplatfactoModel)
...
SplatfactoCustomMethod = MethodSpecification(
    config=TrainerConfig(
        method_name="splatfacto-custom",
        pipeline=VanillaPipelineConfig(
            datamanager=FullImageDatamanagerConfig(
                dataparser=NerfstudioDataParserConfig(load_3D_points=True),
                cache_images_type="uint8",
            ),
            model=SplatfactoCustomModelConfig(),
        ),
        optimizers={
            "means": {
    ...
    description="Splatfacto with position regularization (hull anchor)",
)